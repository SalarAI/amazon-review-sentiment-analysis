...
The DeBERTa-v3 experiment uses the same balanced split and rank-16 LoRA strategy as the RoBERTa LoRA run. The initial training session in `deberta_v3_keras.ipynb` exceeded Kaggle's time limit. Its saved `BackupAndRestore` state was then attached to `deberta-v3-resume.ipynb`, where the same run continued and finished. The final model reaches **0.6757 validation micro-F1** with only **2.22%** of its parameters trainable.

The repository keeps the original training notebook, [`deberta_v3_keras.ipynb`](notebooks/part3/deberta_v3_keras.ipynb), and the final executed continuation with all saved outputs, [`deberta-v3-resume.ipynb`](notebooks/part3/deberta-v3-resume.ipynb). A separate resume template is not needed.
...
All three submissions were evaluated, and each submission score matched its corresponding validation micro-F1:

| Model | Validation micro-F1 | Submission score |
|---|---:|---:|
| RoBERTa-base LoRA, rank 16 | 0.6748 | 0.6748 |
| DeBERTa-v3-base LoRA, rank 16 | 0.6757 | 0.6757 |
| **RoBERTa-base Full + Ordinal** | **0.6948** | **0.6948** |

Submission notebooks:

- [`roberta_lora_test_inference_submission.ipynb`](notebooks/part3/roberta_lora_test_inference_submission.ipynb)
- [`deberta_v3_test_inference_submission.ipynb`](notebooks/part3/deberta_v3_test_inference_submission.ipynb)
- [`roberta_full_test_inference_submission.ipynb`](notebooks/part3/roberta_full_test_inference_submission.ipynb)
...
3. Run one of the model experiments:
   ...
   - For the recorded DeBERTa-v3 experiment, start with `notebooks/part3/deberta_v3_keras.ipynb`; `notebooks/part3/deberta-v3-resume.ipynb` contains the completed continuation from its attached timeout checkpoint
